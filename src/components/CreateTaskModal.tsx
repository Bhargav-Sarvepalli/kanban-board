...
function CreateTaskModal({ onClose, onTaskCreated, userId, defaultStatus, workspaceId }: Props) {
  ...
  // Fetch workspace members + their profiles when in a workspace
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Also include the workspace owner (current user) if not already in members
      const ownerInMembers = wm.some(m => m.user_id === userId)
      const allMembers: Member[] = wm.map(m => ({
        user_id: m.user_id,
        email: m.email,
      ...
      if (!ownerInMembers) {
        ...
        if (ownerProfile) {
          allMembers.unshift({ user_id: userId, email: ownerProfile.email, profile: ownerProfile })
        }
      }
  ...
  const handleClose = () => {
    if (title.trim() || description.trim()) setShowConfirm(true)
    else onClose()
  }
    ...
    <>
            ...
            <div style={{ padding: '28px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
                ...
                <button onClick={handleClose} style={{
                  ...
                  width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px',
                }}>✕</button>
              </div>
                  ...
                  <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                    <div style={{ marginBottom: '24px' }}>
                      <input
                        ...
                        style={{
                          ...
                          fontFamily: 'Space Grotesk', fontWeight: 600, outline: 'none', letterSpacing: '-0.02em',
                        }}
                    ...
                    <div style={{ marginBottom: '24px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                        ...
                        <button
                          onClick={handleGenerateDescription} disabled={generatingDesc || !title.trim()}
                          style={{
                            background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.25)',
                            borderRadius: '6px', color: '#a78bfa', cursor: 'pointer',
                            fontSize: '11px', fontFamily: 'Space Grotesk', padding: '4px 12px',
                      ...
                      <textarea
                        ...
                        placeholder="Describe what needs to happen..." rows={4}
                        style={{
                          width: '100%', background: 'rgba(255,255,255,0.04)',
                          border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '14px',
                          color: 'rgba(255,255,255,0.8)', fontSize: '13px', fontFamily: 'Space Grotesk',
                    ...
                    <motion.button
                      ...
                      onClick={() => { if (!title.trim()) { setError('Please enter a title first'); return } setError(''); setStep(2) }}
                      style={{
                        width: '100%',
                  ...
                  <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}>
                    ...
                    <div style={{ marginBottom: '20px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                        ...
                        <button onClick={handleSuggestPriority} disabled={suggestingPriority}
                          style={{ background: 'none', border: 'none', color: '#a78bfa', cursor: 'pointer', fontSize: '11px', fontFamily: 'Space Grotesk' }}>
                        ...
                        {priorityOptions.map(opt => (
                          <button key={opt.value} onClick={() => setPriority(opt.value as 'low' | 'normal' | 'high')}
                            style={{
                              ...
                              fontWeight: priority === opt.value ? 700 : 500, transition: 'all 0.15s',
                    ...
                    {workspaceId && members.length > 0 && (
                      <div style={{ marginBottom: '20px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                          ...
                          <button
                            ...
                            style={{
                              ...
                              border: `1px solid ${assigneeId === null ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.12)'}`,
                              background: assigneeId === null ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.03)',
                              color: assigneeId === null ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.4)',
                              cursor: 'pointer', fontSize: '11px', fontFamily: 'Space Grotesk',
                        ...
                        {/* Selected assignee confirmation */}
                        {assignee && (
                          <p style={{ color: 'rgba(139,92,246,0.7)', fontSize: '11px', fontFamily: 'Space Grotesk', marginTop: '7px' }}>
                            Assigned to {assignee.profile?.full_name ?? assignee.email}
                          </p>
                        )}
                      </div>
                    )}
                        ...
                        {statusOptions.map(opt => (
                          <button key={opt.value} onClick={() => setStatus(opt.value as Status)}
                            style={{
                              ...
                              cursor: 'pointer', fontSize: '12px', fontFamily: 'Space Grotesk',
                              fontWeight: status === opt.value ? 600 : 400, textAlign: 'left',
                              transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: '8px',
                            }}
                          >
                            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: opt.color, boxShadow: status === opt.value ? `0 0 6px ${opt.color}` : 'none', flexShrink: 0 }} />
                            {opt.label}
                          </button>
                        ))}
                      </div>
                      ...
                      <div>
                        ...
                        <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)}
                          style={{
                            ...
                            border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px', padding: '10px 12px',
                            color: 'rgba(255,255,255,0.75)', fontSize: '12px', fontFamily: 'Space Grotesk',
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          {recurringOptions.map(opt => {
                            ...
                            return (
                              <button key={opt.value ?? 'none'} onClick={() => setRecurring(opt.value as 'weekly' | 'monthly' | null)}
                                style={{
                                  ...
                                  cursor: 'pointer', fontSize: '11px', fontFamily: 'Space Grotesk',
                                  textAlign: 'left', fontWeight: isSelected ? 600 : 400, transition: 'all 0.15s',
                                  display: 'flex', alignItems: 'center', gap: '6px',
                                  boxShadow: isSelected ? `0 0 10px ${opt.glow}` : 'none',
                                }}
                              >
                                <span style={{ fontSize: '13px', color: isSelected ? opt.color : 'rgba(255,255,255,0.4)' }}>{opt.icon}</span>
                                {opt.label}
                              </button>
                            )
                          })}
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button onClick={() => setStep(1)}
                        style={{
                          ...
                          border: '1px solid rgba(255,255,255,0.12)', borderRadius: '12px', padding: '13px',
      ...
      {showConfirm && (
        <ConfirmDialog
          ...
          onConfirm={onClose} onCancel={() => setShowConfirm(false)}
        />
      )}
    </>
  )
