...
export default function FlowGraph({ workspaceId, onBranchClick }: FlowGraphProps) {
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '3px' }}>
            {(['person', 'feature'] as const).map(v => (
              <button key={v} onClick={() => setViewBy(v)}
                style={{ padding: '5px 14px', borderRadius: '6px', border: 'none', background: viewBy === v ? 'rgba(139,92,246,0.3)' : 'transparent', color: viewBy === v ? '#a78bfa' : 'rgba(255,255,255,0.4)', cursor: 'pointer', fontSize: '11px', fontFamily: 'Space Mono', letterSpacing: '0.08em', transition: 'all 0.15s', textTransform: 'uppercase' as const }}>
                {v}
              </button>
            ))}
          </div>
      ...
      {/* ── BRANCH LEGEND PILLS ── */}
      <div style={{ display: 'flex', gap: '8px', padding: '10px 24px', borderBottom: '1px solid rgba(255,255,255,0.04)', overflowX: 'auto', flexShrink: 0, alignItems: 'center' }}>
        {branches.map(b => (
      ...
      <div
        ...
        onMouseDown={(e) => {
          setIsDragging(true)
          setDragStart({ x: e.clientX, scroll: scrollRef.current?.scrollLeft ?? 0 })
        }}
        onMouseMove={(e) => {
          if (!isDragging) return
          e.preventDefault()
          const dx = e.clientX - dragStart.x
          if (scrollRef.current) scrollRef.current.scrollLeft = dragStart.scroll - dx
        }}
      ...
      >
        <svg width={layout.svgWidth} height={SVG_HEIGHT} style={{ display: 'block' }}>
          ...
          {/* Zone tints */}
          <rect x={0}     y={0} width={pastX}                    height={SVG_HEIGHT} fill="rgba(34,197,94,0.012)" />
          <rect x={pastX} y={0} width={nowX - pastX}             height={SVG_HEIGHT} fill="rgba(139,92,246,0.018)" />
          <rect x={nowX}  y={0} width={layout.svgWidth - nowX}   height={SVG_HEIGHT} fill="rgba(255,255,255,0.006)" />

          {/* Zone dividers */}
          <line x1={pastX} y1={40} x2={pastX} y2={SVG_HEIGHT - 30} stroke="rgba(255,255,255,0.07)" strokeWidth={1} strokeDasharray="4 4" />
          <text x={pastX + 8} y={56} fill="rgba(255,255,255,0.15)" fontSize={9} fontFamily="Space Mono" letterSpacing="2">PAST</text>
          <line x1={nowX}  y1={40} x2={nowX}  y2={SVG_HEIGHT - 30} stroke="rgba(139,92,246,0.25)" strokeWidth={1} strokeDasharray="4 4" />
          <text x={nowX + 8}  y={56} fill="rgba(139,92,246,0.5)"   fontSize={9} fontFamily="Space Mono" letterSpacing="2">NOW</text>

          {/* Trunk glow */}
          <line x1={60} y1={TRUNK_Y} x2={trunkEnd} y2={TRUNK_Y}
            stroke="rgba(139,92,246,0.12)" strokeWidth={14} strokeLinecap="round" filter="url(#trunkGlow)" />
          {/* Trunk line */}
          <line x1={60} y1={TRUNK_Y} x2={trunkEnd} y2={TRUNK_Y}
            stroke="url(#trunkGrad)" strokeWidth={2.5} strokeLinecap="round" />
          {/* Trunk arrow */}
          <polygon points={`${trunkEnd + 14},${TRUNK_Y} ${trunkEnd},${TRUNK_Y - 7} ${trunkEnd},${TRUNK_Y + 7}`}
            ...
            return (
              <g key={label[0]}>
                ...
                <text x={x} y={TRUNK_Y + 5} textAnchor="middle"
                  fill={isPast ? 'rgba(0,0,0,0.85)' : isNow ? 'white' : 'rgba(255,255,255,0.2)'}
                  fontSize={12} fontFamily="Space Mono">
                  {isPast ? '✓' : isNow ? '⚡' : '○'}
                </text>
                {label.map((line, li) => (
                  <text key={li} x={x} y={TRUNK_Y + TRUNK_NODE_R + 16 + li * 13}
                    textAnchor="middle"
          ...
          {layout.branches.map(({ branch, above, originX, branchY, nodes, endX }) => {
            ...
            return (
              ...
              >
                ...
                {/* Progress bar along branch */}
                <line
                  x1={originX + 50} y1={above ? branchY - 5 : branchY + 5}
                  x2={originX + 50 + (endX - originX - 50) * (branch.progress / 100)}
                  y2={above ? branchY - 5 : branchY + 5}
                  stroke={branch.color} strokeWidth={2.5} strokeOpacity={0.35} strokeLinecap="round"
                />

                {/* Branch label chip */}
                <foreignObject
                  x={originX + 46}
                  y={above ? branchY - 50 : branchY + 14}
                  width={170} height={34}
                  ...
                  return (
                    <g key={task.id}
                      onMouseEnter={(e) => {
                        ...
                        setTooltip({ x: e.clientX, y: e.clientY, task, branch })
                      }}
                    ...
                    >
                      ...
                      <circle cx={x} cy={y} r={nR}
                        ...
                        stroke={border}
                        strokeWidth={overdue || isNH ? 2.5 : 2}
                      ...
                      <text x={x} y={y + 4.5} textAnchor="middle"
                        ...
                        fontSize={standupMode ? 13 : 11} fontFamily="Space Mono">
                        {icon}
                      </text>

                      {/* Task title */}
                      <text x={x} y={above ? y - nR - 10 : y + nR + 15}
                        textAnchor="middle" fill="rgba(255,255,255,0.5)"
                        fontSize={standupMode ? 11 : 9} fontFamily="Space Grotesk"
                      ...
                      {/* Due date */}
                      {task.due_date && (
                        <text x={x} y={above ? y - nR - 22 : y + nR + 27}
                          textAnchor="middle"
                          fill={overdue ? '#ef444466' : 'rgba(255,255,255,0.18)'}
        ...
        {tooltip && (
          <div style={{
            ...
            pointerEvents: 'none', zIndex: 9999, width: '220px',
      ...
      {/* ── SCROLL HINT ── */}
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px', padding: '8px', borderTop: '1px solid rgba(255,255,255,0.04)', flexShrink: 0 }}>
        <span style={{ color: 'rgba(255,255,255,0.1)', fontSize: '10px', fontFamily: 'Space Mono', letterSpacing: '0.1em' }}>← DRAG TO EXPLORE →</span>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          {[
            { label: 'PAST',     color: '#22c55e' },
            { label: 'NOW',      color: '#8b5cf6' },
            { label: 'UPCOMING', color: 'rgba(255,255,255,0.15)' },
          ].map(s => (
            <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <div style={{ width: '20px', height: '2px', background: s.color, borderRadius: '1px' }} />
              <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: '9px', fontFamily: 'Space Mono' }}>{s.label}</span>
            </div>
          ))}
        </div>
      </div>
